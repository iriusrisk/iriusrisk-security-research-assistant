...
def new():
    ...
    component = Component(ref=component_ref, name=component_name, description=component_desc, category=category)
    template.set_component(component)

    # Now we start building the component in the temporal storage
    risk_pattern_ref = PREFIX_RISK_PATTERN + get_company_name_prefix() + component_name.upper()
    risk_pattern_ref = re.sub(r"[^a-zA-Z0-9]+", "-", risk_pattern_ref)
    risk_pattern = RiskPattern(ref=risk_pattern_ref, name=component_name, description=component_desc)
    template.set_risk_pattern(risk_pattern)
    ...
    print(tx.get_values())
    print(tx.get_string_values())

    print(template)
    write_current_component(template)

    print(f"Component {component.ref} created")
