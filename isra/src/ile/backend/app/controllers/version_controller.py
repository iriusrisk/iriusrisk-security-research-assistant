...
async def import_library_to_version(version_ref: str, files: List[UploadFile] = File(...), version_facade: VersionFacade = Depends(get_version_facade)) -> None:
...
async def import_libraries_from_folder(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> None:
...
async def export_version_to_folder_xml(version_ref: str, format: str, version_facade: VersionFacade = Depends(get_version_facade)) -> None:
...
async def get_suggestions_for_element(version_ref: str, body: SuggestionRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRSuggestions:
...
async def create_library(version_ref: str, library_request: LibraryRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRLibrary:
    """Create library"""
    return version_facade.create_library(version_ref, library_request.library_ref)


@router.put("/version/{version_ref}/library")
async def increment_library_revision(version_ref: str, library_request: LibraryRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> None:
...
async def delete_library(version_ref: str, library_request: LibraryRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete library"""
    version_facade.delete_library(version_ref, library_request.library_ref)


@router.get("/version/{version_ref}/reference")
async def list_references(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRReference]:
    """List references"""
    return version_facade.list_references(version_ref)


@router.get("/version/{version_ref}/reference/{uuid}")
async def get_reference(version_ref: str, uuid: str, version_facade: VersionFacade = Depends(get_version_facade)) -> IRReference:
    """Get reference by UUID"""
    return version_facade.get_reference(version_ref, uuid)


@router.post("/version/{version_ref}/reference")
async def add_reference(version_ref: str, body: ReferenceRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRReference:
    """Add reference"""
...
async def delete_references(version_ref: str, body: List[IRReference], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete references"""
    for ref in body:
...
async def list_categories(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRCategoryComponent]:
    """List categories"""
    return version_facade.list_categories(version_ref)


@router.post("/version/{version_ref}/category")
async def add_category(version_ref: str, body: CategoryRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRCategoryComponent:
    """Add category"""
...
async def delete_category(version_ref: str, body: List[IRCategoryComponent], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete categories"""
...
async def list_supported_standards(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRSupportedStandard]:
    """List supported standards"""
    return version_facade.list_supported_standards(version_ref)


@router.post("/version/{version_ref}/supportedStandard")
async def add_supported_standard(version_ref: str, body: SupportedStandardRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRSupportedStandard:
...
async def delete_supported_standard(version_ref: str, body: List[IRSupportedStandard], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
...
async def list_standards(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRStandard]:
    """List standards"""
    return version_facade.list_standards(version_ref)


@router.post("/version/{version_ref}/standard")
async def add_standard(version_ref: str, body: StandardRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRStandard:
    """Add standard"""
...
async def delete_standard(version_ref: str, body: List[IRStandard], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete standards"""
    for req in body:
...
async def list_controls(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRControl]:
    """List controls"""
    return version_facade.list_controls(version_ref)


@router.post("/version/{version_ref}/control")
async def add_control(version_ref: str, body: ControlRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRControl:
    """Add control"""
    return version_facade.add_control(version_ref, body)


@router.put("/version/{version_ref}/control")
async def update_control(version_ref: str, body: ControlUpdateRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRControl:
    """Update control"""
    return version_facade.update_control(version_ref, body)


@router.delete("/version/{version_ref}/control")
async def delete_controls(version_ref: str, body: List[IRControl], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete controls"""
    for req in body:
...
async def add_reference_to_control(version_ref: str, body: ReferenceItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRControl:
...
async def delete_reference_from_control(version_ref: str, body: ReferenceItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRControl:
...
async def add_standard_to_control(version_ref: str, body: StandardItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRControl:
...
async def delete_standard_from_control(version_ref: str, body: StandardItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRControl:
...
async def list_weaknesses(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRWeakness]:
    """List weaknesses"""
    return version_facade.list_weaknesses(version_ref)


@router.post("/version/{version_ref}/weakness")
async def add_weakness(version_ref: str, body: WeaknessRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRWeakness:
    """Add weakness"""
    return version_facade.add_weakness(version_ref, body)


@router.put("/version/{version_ref}/weakness")
async def update_weakness(version_ref: str, body: WeaknessRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRWeakness:
    """Update weakness"""
    return version_facade.update_weakness(version_ref, body)


@router.delete("/version/{version_ref}/weakness")
async def delete_weakness(version_ref: str, body: List[IRWeakness], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete weaknesses"""
    for req in body:
...
async def add_reference_to_weakness(version_ref: str, body: ReferenceItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRWeakness:
...
async def delete_reference_from_weakness(version_ref: str, body: ReferenceItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRWeakness:
...
async def list_usecases(version_ref: str, version_facade: VersionFacade = Depends(get_version_facade)) -> List[IRUseCase]:
    """List use cases"""
    return version_facade.list_usecases(version_ref)


@router.post("/version/{version_ref}/usecase")
async def add_usecase(version_ref: str, body: UsecaseRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRUseCase:
    """Add use case"""
...
async def delete_usecase(version_ref: str, body: List[IRUseCase], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete use cases"""
    for usecase in body:
...
async def add_threat(version_ref: str, body: ThreatRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRThreat:
    """Add threat"""
    return version_facade.add_threat(version_ref, body)


@router.put("/version/{version_ref}/threat")
async def update_threat(version_ref: str, body: ThreatUpdateRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRThreat:
    """Update threat"""
    return version_facade.update_threat(version_ref, body)


@router.delete("/version/{version_ref}/threat")
async def delete_threat(version_ref: str, body: List[IRThreat], version_facade: VersionFacade = Depends(get_version_facade)) -> None:
    """Delete threats"""
    for req in body:
...
async def add_reference_to_threat(version_ref: str, body: ReferenceItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRThreat:
...
async def delete_reference_from_threat(version_ref: str, body: ReferenceItemRequest, version_facade: VersionFacade = Depends(get_version_facade)) -> IRThreat:
